...
void UWidgetSlotGrid::SetWidgetSlotGrid(UItemContainer * itemCon)
{
	...
	//m_BaseIcon=
	if (IsValid(m_CoverImage->Brush.GetResourceObject()))
	{
	m_BaseIcon = Cast<UTexture2D> (m_CoverImage->Brush.GetResourceObject());
	ShowCoverImage();
	}
}
...
void UWidgetSlotGrid::SetCoverImageToDefault()
{
	if(IsValid(m_BaseIcon))
		m_CoverImage->SetBrushFromTexture(m_BaseIcon);
}

void UWidgetSlotGrid::SetCoverImage(UTexture2D * newTexture)
{
	m_CoverImage->SetBrushFromTexture(newTexture);
}

void UWidgetSlotGrid::ShowCoverImage()
{
	m_CoverImage->SetVisibility(ESlateVisibility::SelfHitTestInvisible);//체크해볼것
}

void UWidgetSlotGrid::HideCoverImage()
{
	m_CoverImage->SetVisibility(ESlateVisibility::Hidden);
}

void UWidgetSlotGrid::SetCoverImageTint(const FLinearColor& colorWant)
{
	m_CoverImage->SetColorAndOpacity(colorWant);
}

void UWidgetSlotGrid::SetCoverImageTintDefault()
{
	m_CoverImage->SetColorAndOpacity(FLinearColor::White);
}
