...
public:
	FStatF():
		m_fStatMaximumLimit((int)E_GLOBAL_CONST::MAXSTAT),
		m_fValue(1.f),
...
public:
	...
	FORCEINLINE E_ITEMTIER SetRandomTierRoll()  const
	{
...
class AGoldGroundActor;

USTRUCT(BlueprintType)
struct  FDropGoldData
{//이것도 액수만 써놓고
	//특정액수마다 크기 다른 메쉬로 구현할것
public:
	GENERATED_BODY()
	/*FDropGoldData()
	{
		m_GainGoldAmount = 0.0f;
	}
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly)
	TSubclassOf<AGoldGroundActor> m_DropGold;
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, meta = (ClampMin = "0", ClampMax = "99999", UIMin = "0", UIMax = "99999"))
	float m_GainGoldAmount;*/
};
USTRUCT(BlueprintType)
struct FOption
{
...
public:
	FOption()
	{
		...
		m_OptionName = FText::FromString("NoName");
		m_PlusMinus=FText::FromString("");
		m_Percent = FText::FromString("");
		m_IsPercV = false;
	}
...
public:
	...
	FText ToText(int floatCount=1) const
	{
		...
		Args.Add(m_Percent);
		auto FormatT= FTextFormat(FText::FromString("{0}{1}{2}{3}"));
		return FText::Format(FormatT, Args);
	}
};
