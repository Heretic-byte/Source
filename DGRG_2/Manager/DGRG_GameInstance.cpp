...
void UDGRG_GameInstance::Init()
{
	...
	m_DgManager = NewObject<UDungeonManager>(m_CDungeonManager, NAME_None, RF_NoFlags, m_CDungeonManager->GetDefaultObject<UDungeonManager>());
	//
	//여기서 셋
...
APlayerCharacter * UDGRG_GameInstance::GetPlayerCharacter()
{
	return m_PlayerChar;
}


UWidgetMasterCanvas * UDGRG_GameInstance::GetMasterCanvas()
{
	return m_MasterCanvas;
}
void UDGRG_GameInstance::SetMasterCanvas(UWidgetMasterCanvas * masterCanvas)
{
	m_MasterCanvas = masterCanvas;
}
...
void UDGRG_GameInstance::SetPlayerActor(ADGRG_CombatCharacter * combatP)
{
	...
	m_PlayerChar = Cast<APlayerCharacter>(combatP);
}
