...
// Sets default values
ADGRG_BaseCharacter::ADGRG_BaseCharacter(const FObjectInitializer& ObjectInitializer)
...
void ADGRG_BaseCharacter::SetCurrentHealthToMax()
{
	m_CurrentHealth = GetStatFinal().m_HealthPoint.GetFinalValue();
...
void ADGRG_BaseCharacter::PlayAnim(int index)
{
	...
	{
		PlayAnimMontage(AnimInst->m_Motions[index],1.5f);//GetStatFinal().m_AttackSpeed
	}
	else
	{
		ResetCombo();
